---
title: (English) scriptCTF 2025 - Python Jail
date: 2025-08-17 00:00:00 +0007
categories: [Write-up]
tags: [python, English]

image:
  path: /assets/img/2025-08-17-PythonJailScriptCTF/cover.png

description: "My write-up for Misc - Modulo (Python Jail) challenge"
---

## About the challenge

> Description: Modulo is so cool! (by @NoobMaster)
...
If we had zero restrictions, we could do something like this to pop a shell! We’ll need to work around the restrictions though.
...
We got this because if we let x be an integer, we got:
- Bitwise NOT => `~x = -x-1`
- Negate => `-~x = x+1`
...
Since only **one comparison** is allowed, use the **walrus operator (`:=`)** to assign the result of `('@' < 'A')` (which evaluates to `True`, i.e. `1`) to the variable `I`:
...
Define a helper function to construct any integer `n` using only `I` and the unary increment operator `-~`:
...
> Each `-~` increases the value by 1, so `-~-~I == 3`.

---

### 3. Build Arbitrary Strings

Use `%c%c%c...` format strings to build any string:
...
Use `getattr(c, ...)` to dynamically access attributes:
...
Combine all steps into a single tuple:

```python
PAYLOAD = f"({init},{step3})"
```

Here, `init` initializes `I`, and `step3` launches the shell.
...
step1 = getattr_call("c", "__self__")
step2 = f"c({step1},{imp_name})({os_mod})"
step3 = f"c({step2},{sys_name})({sh_str})"
